...
public abstract class ParserBase {
  ...
  public void setDocumentByUrl(Stream<String> url) {
    document =
        url.filter(
                element ->
                    element != null && !element.equals("") && !element.equals(Util.EXTERNAL_PAGE))
            .map(
                element -> {
                  final var tempName =
                      switch (sheetType) {
  ...
  private Stream<
          AbstractMap.SimpleImmutableEntry<
              String, AbstractMap.SimpleImmutableEntry<String, String>>>
  ...
  public Stream<
          AbstractMap.SimpleImmutableEntry<
              String, AbstractMap.SimpleImmutableEntry<String, String>>>
      parsePage(Stream<SheetSource> sheetSourceStream) {
    final var stream =
        sheetSourceStream
            .parallel()
            // element is a row in table sheet_source
            .filter(element -> element.getSheetSourceType().equals(this.sheetType))
            .flatMap(
                (elementToBeParsed) ->
                    parseElementByTag(
