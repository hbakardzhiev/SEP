...
/**
 * We follow somewhat the State design pattern
 * by creating a base abstract parser from which we create
 * further classes.
 *
 */
abstract public class ParserBase {
  ...
  /**
   * Dictionary which holds the unique name of the page
   * eg. CR000001, ProjectName01 - SW Tooling, E0011 LocationId002 as key
   * and the parsed HTML content as key
   *
   */
  @Getter
  @Setter
  ...
  /**
   *  Takes the stream of urls eg. locations of the webpages,
   *  goes through each of them and saves them in the document hashmap.
   *
  ...
  /**
   * Goes through the page and finds all HTML elements that satisfy the
   * tag, id capture. Returns the pair of unique name of the page with value (id on the page with key
   * the html value of the element).
   *
  ...
  /**
   * Takes all elements from the sheetSource filters only the relevant ones for the page
   * parses them and returns usable data in the form of Stream
   * @param sheetSourceStream Stream of SheetSources tells it which page to gather
   * @return key - unique name of the document, value - key, value pair that holds the id and text value
   * @throws IOException
   */
